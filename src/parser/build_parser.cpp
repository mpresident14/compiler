...
vector<bool> bitOr(const vector<bool> &lhs, const vector<bool> &rhs) {
  size_t len = lhs.size();
...
void addRhses(queue<DFARule> &ruleQueue, const DFARule &fromRule,
  ...
  for (int concreteType : grammarData.variables[nextSymbol].concreteTypes) {
    ruleQueue.push(DFARule{concreteType,
                           grammarData.concretes[concreteType].argSymbols, 0,
                           newLookahead});
  }
}
...
void epsilonTransition(DFARuleSet &ruleSet, const GrammarData &grammarData,
                       const vector<BitSetToks> &firsts) {
...
vector<DFA_t::Node *> createTransitions(DFA_t &dfa, DFA_t::Node *node,
                                        const GrammarData &grammarData,
                                        const vector<BitSetToks> &firsts) {
  ...
  for (size_t i = 0; i < numSymbols; ++i) {
    ...
    DFA_t::Node *newNode = dfa.addTransition(node, indexToSymbol(i, numVars),
                                             move(transitionRules));
    if (newNode) {
...
DFA_t initDFA(const GrammarData &grammarData,
              const vector<BitSetToks> &firsts) {
  int rootType = grammarData.variables[S].concreteTypes[0];
  DFARuleSet firstSet = {DFARule{SCONC,
                                 grammarData.concretes[rootType].argSymbols, 0,
...
void streamRule(std::ostream &out, const DFARule &rule,
                const GrammarData &grammarData) {
...
void shiftReduceConflict(const DFARule &reduceRule, const DFARule &shiftRule,
                         const GrammarData &grammarData) {
...
void reduceReduceConflict(const DFARule &reduceRule1,
...
void findShiftReduceConflicts(const DFARule &reducibleRule, int rulePrecedence,
...
RuleData condenseRuleSet(const DFARuleSet &ruleSet,
  ...
  auto ruleIter = find_if(reducibleRule->symbols.crbegin(),
                          reducibleRule->symbols.crend(), isToken);
  ...
  findShiftReduceConflicts(*reducibleRule, rulePrecedence, ruleSet,
                           grammarData);
...
string ruleDataToCode(const RuleData &ruleData) {
  ...
  if (ruleData.reducibleRule.has_value()) {
    ...
    code << '{';
    for_each(rule.symbols.cbegin(), rule.symbols.cend(),
             [&code](int n) { code << to_string(n) << ','; });
    code << "},";
    ...
    code << '{';
    for_each(rule.lookahead.cbegin(), rule.lookahead.cend(),
             [&code](bool b) { code << to_string(b) << ','; });
    code << "}}},";
  } else {
...
void condensedDFAToCode(ostream &out, const GrammarData &grammarData) {
  buildParserDFA(grammarData)
      .streamAsCode(
          ...
          ruleDataToCode, [](int n) { return to_string(n); });
}
