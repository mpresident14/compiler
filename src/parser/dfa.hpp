...
template <typename V, typename T, typename HashV = std::hash<V>,
          typename EqV = std::equal_to<V>, typename HashT = std::hash<T>,
          typename EqT = std::equal_to<T>>
class DFA {
public:
  template <typename V2, typename T2, typename HashV2, typename EqV2,
            typename HashT2, typename EqT2>
  friend class DFA;
  ...
  public:
    ...
    const std::unordered_map<T, Node *, HashT, EqT> &
  ...
  DFA(V value)
      : root_(new Node(value)), valueToNode_{{&root_->value_, root_}},
        size_(1) {}
  ...
  void streamAsCode(std::ostream &out, const std::string &valueType,
                    const std::string &tranType, const F1 &convertValue,
                    const F2 &valueToStr, const F3 &tranToStr) const {
