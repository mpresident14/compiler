...
struct GrammarData GRAMMAR_DATA = {
    ...
    {{"S", "", {SCONC}, ""},
     {"Regex",
      "",
      {REGEX_ALT, REGEX_CONCATS, REGEX_STAR, REGEX_NOT, REGEX_RANGE,
       REGEX_GROUP, REGEX_CHAR, REGEX_BRACKET_CHAR, REGEX_DOT},
      ""},
     {"Alt",
      "",
      {
...
void *constructObj(int concrete, StackObj *args) {
  ...
  case REGEX_NOT:
    return new Regex *(*static_cast<Regex **>(args[0].releaseObj()));
  case REGEX_RANGE:
    return new Regex *(new Range(*static_cast<char *>(args[1].releaseObj()),
  ...
  case NOT_RANGE:
    return new Regex *(
        new Not(new Range(*static_cast<char *>(args[2].releaseObj()),
                          *static_cast<char *>(args[4].releaseObj()))));
  case ALTS_REGEX:
    return new RegexVector(
        RegexVector(*static_cast<Regex **>(args[0].releaseObj()),
                    *static_cast<Regex **>(args[2].releaseObj())));
  case ALTS_ALTS:
    return new RegexVector(
        RegexVector(move(*static_cast<RegexVector *>(args[0].releaseObj())),
                    *static_cast<Regex **>(args[2].releaseObj())));
  case CONCATS_REGEX:
    return new RegexVector(
        RegexVector(*static_cast<Regex **>(args[0].releaseObj()),
...
int tryReduce(const CondensedNode *node, int nextToken, vector<StackObj> &stk,
