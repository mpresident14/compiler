...
RgxPtr Alt::getDeriv(char c) const {
  ...
  std::transform(rgxs_.cbegin(), rgxs_.cend(), std::back_inserter(derivs), [c](const RgxPtr rPtr) {
...
RgxPtr Concat::getDeriv(char c) const {
  ...
  if (rgxs_[0]->isNullable()) {
    ...
    return std::make_shared<Alt>(Alt({Concat(std::move(rest1)).getDeriv(c),
...
RgxPtr Not::getDeriv(char c) const { return std::make_shared<Not>(Not(rgx_->getDeriv(c))); }
